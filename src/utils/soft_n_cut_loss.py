...
def soft_n_cut_loss(inputs, segmentations,k):
    # We don't do n_cut_loss batch wise -- split it up and do it instance wise
    loss = 0
    for i in range(inputs.shape[0]):
        # flatten_image = torch.mean(inputs[i], dim=0)
        # flatten_image = flatten_image.reshape(flatten_image.shape[0)
        row = inputs[i].shape[1]
        col = inputs[i].shape[2]
        loss += soft_n_cut_loss_(inputs[i].flatten(), segmentations[i], k, row, col)
