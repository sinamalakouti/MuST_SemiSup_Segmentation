...
def eval_per_subjectUnet(model, device, threshold, cfg, data_mode, val_loader):
...
def Unet_train_val(dataset, n_epochs, wmh_threshold, output_dir, args, cfg, seed):
    ...
    wandb.run.name = "{}_UNET_{}_{}_supRate{}_seed{}_".format(cfg.experiment_mode, "trainALL2018", cfg.val_mode,
    ...
    unet = Baseline_Unet.Unet(inputs_dim, outputs_dim, kernels, strides, cfg)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if epoch % 2 == 0:
            ...
            val_dice = evaluate(unet, device, wmh_threshold, output_image_dir, epoch, cfg, val_loader)
    ...
    evaluate(unet, device, wmh_threshold, output_image_dir, cfg.n_epochs, cfg, val_loader)


def evaluate(unet, device, wmh_threshold, output_image_dir, epoch, cfg, val_loader):
